...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ...
    className={cn(
      ...
      "data-[state=open]:bg-[var(--accent-dim)]",
      inset && "pl-8",
      className
    )}
    {...props}
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.SubContent
    ref={ref}
    className={cn(
      "z-[9999] min-w-[8rem] overflow-hidden p-1",
      "bg-[var(--bg-card)] text-[var(--text-primary)]",
      "border border-[var(--border-solid)]",
      "[clip-path:var(--clip-chamfer-sm)]",
      "shadow-[0_4px_20px_rgba(0,0,0,0.5),0_0_10px_var(--border-glow)]",
...
>(({ className, sideOffset = 4, variant = "fantasy", ...props }, ref) => (
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <DropdownMenuPrimitive.ItemIndicator>
        <Circle className="h-2 w-2 fill-[var(--accent-primary)]" />
      </DropdownMenuPrimitive.ItemIndicator>
    </span>
    {children}
  </DropdownMenuPrimitive.RadioItem>
...
const DropdownMenuShortcut = ({
  ...
  return (
    <span
      className={cn("ml-auto text-xs tracking-widest text-[var(--text-dim)]", className)}
      {...props}
    />
  )
}
